...
const SplitContainerItem : Script = preload("res://addons/multi_spliter_container/split_container_item.gd")
const SplitButton : Texture = preload("res://addons/multi_spliter_container/icon/MultiSpliterButton.svg")
			...
			if x.visible and !x.is_queued_for_deletion():
				if x is SplitContainerItem:
					...
					if Engine.is_editor_hint():
						...
						var current : String = _undoredo.get_current_action_name()
						if current == "Create Node":
							var _ur : UndoredoSplit = UndoredoSplit.new()

							_ur.object = container

							undoredo.create_action(current + "2",UndoRedo.MERGE_ALL, null, true)
							undoredo.add_do_method(self, "_undoredo_do", _ur)
							undoredo.add_undo_method(self, "_undoredo_undo", _ur)
							undoredo.force_fixed_history()
							undoredo.add_do_reference(_ur)

							if _last_undoredo:
								undoredo.add_undo_reference(_last_undoredo)
